...
namespace Native.App.Export
{
	...
	{	
		...
		{	
			return "9,io.nekonya.evemarket.bot";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("io.nekonya.evemarket.bot", 1, 9, "EVE市场姬", "3.0.3", 5, "yomunsam", "EVE市场姬-Nekonya Studio", authCode);	
			...
			AppData.UnityContainer.RegisterInstance<CQLog> ("io.nekonya.evemarket.bot", AppData.CQLog);	
		...
		{	
			 ...
			 * Priority: 30000	
			 */	
			if (AppData.UnityContainer.IsRegistered<IGroupMessage> ("group_msg"))	
			{	
				Event_eventGroupMsgHandler += AppData.UnityContainer.Resolve<IGroupMessage> ("group_msg").GroupMessage;	
			}	
		...
		{	
			...
			{	
				CQGroupMessageEventArgs args = new CQGroupMessageEventArgs (AppData.CQApi, AppData.CQLog, 2, 2, "group_msg", "_eventGroupMsg", 30000, subType, msgId, fromGroup, fromQQ, fromAnonymous, msg.ToString(CQApi.DefaultEncoding), false);	
